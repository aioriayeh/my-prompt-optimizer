...
# 2. 介面淨化：只針對提示文字進行隱藏，絕不影響組件顯示
st.markdown("""
    <style>
    /* 隱藏輸入框下方的提示文字 (如 Press Enter to apply) */
...
MODEL_DATA = {
    ...
    "OpenAI": [
        "自定義輸入", "gpt-5.2", "gpt-5-mini", "gpt-4o", "o3-deep-research"
    ],
    "Anthropic Claude": [
        "自定義輸入", "claude-3-5-sonnet-latest", "claude-4-preview"
    ],
    "xAI Grok": [
        "自定義輸入", "grok-4.1", "grok-3"
    ],
    "DeepSeek": [
        "自定義輸入", "deepseek-v4", "deepseek-r1"
    ],
    "NVIDIA": [
        "自定義輸入", "nemotron-3-8b-instruct"
    ],
    "OpenRouter": [
        ...
        with st.spinner("執行中..."):
            try:
                ...
                if "404" in error_detail or "not found" in error_detail.lower():
                    st.info("偵測到模型 ID 錯誤。請點擊左側連結查詢最新 ID。")
